...
```{r}
df <- as.data.frame(list(explanatory=survey[,61], response=survey[,55]))
tbl <- table(df, useNA = 'ifany')
...
df$able <- likely[df$response]

ggplot(df, aes(x=time)) + geom_bar() + theme(axis.text.x = element_text(angle = 315, hjust = 0), plot.margin = margin(10, 40, 10, 10))
```
```{r}

ggplot(df, aes(x=time, fill=forcats::fct_rev(factor(able, levels=c(NA, likely))))) + 
  geom_bar() + 
...
df %>%
    ...
    geom_bar(aes(width=x.width, fill=factor(able)),
             colour = "white", size=1, position=position_fill(reverse = T)) +
    geom_text(aes(label=n),
               position=position_fill(vjust = 0.5)) +
    facet_grid(~ time, space = 'free', scales='free', switch='x') +
